...
class Encoder(nn.Module):
    ...
    def forward(self, x, input_lengths):
        ...
        print(f'encoder convolution output shape: {x.shape}')
        x = x.transpose(1, 2)
        ...
        print(f'encoder lstm output shape: {outputs[0].shape}')
        if plot:
            plt.figure()
            plt.imshow(outputs.float()[0].data.cpu().numpy())
            plt.savefig('train_dnn_outputs.png')
        outputs, _ = nn.utils.rnn.pad_packed_sequence(
            outputs, batch_first=True)
        print(f'encoder output shape: {outputs.shape}')
        exit()
        return outputs
    ...
    def inference(self, x):
        if plot:
            plt.figure()
            plt.imshow(x.float()[0].data.cpu().numpy())
        ...
        if plot:
            plt.figure()
            plt.imshow(outputs.float()[0].data.cpu().numpy())
            plt.savefig('inferencing_encoder(dnn)_outputs.png')
        return outputs
...
class Decoder(nn.Module):
    ...
    def inference(self, memory):
        ...
        mel_outputs, gate_outputs, alignments = self.parse_decoder_outputs(
            mel_outputs, gate_outputs, alignments)
        print(f'alignment_weights shape {alignments.shape}')
        if plot:
